...
export const login = async (
  ...
  try {
    ...
    return next(
      successMessage(200, 'Login successful.', { user: user }, { token: token })
    );
  } catch (error: any) {
    return next(errorMessage(500, 'Internal Server Error.', error.stack));
  }
...
export const elevateUser = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    try {
      ...
      if (requesterData.id === userId) {
        return next(
          errorMessage(403, 'You can not elevate your own privileges.')
        );
      }
      ...
      const adminClearanceLevel = await ClearanceLevel.findOne({
        level: 'Admin',
      });
      ...
      return next(
        successMessage(200, 'User is elevated to admin.', {
          user: newUserState,
        })
      );
    } catch (error: any) {
