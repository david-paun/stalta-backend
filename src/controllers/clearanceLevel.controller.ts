...
import ClearanceLevel, { ClearanceLevelDocument } from '../models/ClearanceLevel.js'; // Import ClearanceLevelDocument if defined separately
import { errorMessage } from '../utils/http-responses/error.js';
import { successMessage } from '../utils/http-responses/success.js';

export const createClearanceLevel = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
...
export const updateClearanceLevel = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
...
export const getClearanceLevels = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const clearanceLevels = await ClearanceLevel.find({});
        return next(successMessage(200, "Clearance Levels sent.", { levels: clearanceLevels }));
    } catch (error:any) {
        return next(errorMessage(500, "Internal Server Error.", error.stack));
    }
};

export const deleteClearanceLevel = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
